...
System::System(ParamDict &theParams, gsl_rng *&the_rg) {
    ...
    if(do_neighbor_grid==1){
        ...
        grid = new NeighborGrid<AOUParticle, 2>(min, max, p, rcut);
        update_neighborgrid();
    }
    ...
    for(int i=0; i<N; i++){
        for(int k=0; k<dim; k++){
            particles[i].self_prop_vel(k) = gsl_ran_gaussian(rg, sqrt(D0/tau));
        }
    }
}
...
void System::do_paramdict_assign(ParamDict &theParams) {
    ...
    if(theParams.is_key("D0")) D0 = std::stod(theParams.get_value("D0"));
...
double System::get_energy_between(AOUParticle &p1, AOUParticle &p2) {
...
double System::get_energy_neighbor_grid() {
    ...
    for(int i=0; i<N; i++){
        ...
        */
        std::vector<AOUParticle *> neighbors = grid->get_neighbors(&particles[i]);
        for(unsigned int j=0; j<neighbors.size(); j++){
            AOUParticle *p2 = neighbors[j];
...
arma::vec System::get_force(AOUParticle &p1) {
...
arma::vec System::get_force_from(AOUParticle &p1, AOUParticle &p2) {
...
arma::vec System::get_force_neighbor_grid(AOUParticle &p1) {

    arma::vec force(dim, arma::fill::zeros);
    /*
    for (auto p2 = grid->get_neighbor_iterator(&p1); !p2.isDone(); p2++){
    */
    std::vector<AOUParticle *> neighbors = grid->get_neighbors(&p1);
    for(unsigned int j=0; j<neighbors.size(); j++){
        AOUParticle *p2 = neighbors[j];  
        double dist = get_dist(p1, *p2);
        if (dist<=rcut) {
...
arma::vec System::get_disp_vec(AOUParticle &p1, AOUParticle &p2) {
...
double System::get_dist(AOUParticle &p1, AOUParticle &p2) {
